...
TEST_CASE("FlowScene triggers connections created or deleted", "[gui]")
{
    ...
    Creation sceneCreation{ "scene.createConnection", [&] { return scene.createConnection(toNode, 0, fromNode, 0); } };
    ...
    Deletion sceneDeletion{ "scene.deleteConnection", [&](Connection &c) { scene.deleteConnection(c); } };
...
TEST_CASE("FlowScene's DataModelRegistry outlives nodes and connections", "[asan][gui]")
{
    ...
    {
      public:
        MockDataModel(int *const &incrementOnDestruction) : incrementOnDestruction(incrementOnDestruction)
        {
        }
    ...
    {
        MockDataModelCreator(int *shouldBeAliveWhenAssignedTo) : shouldBeAliveWhenAssignedTo(shouldBeAliveWhenAssignedTo)
        {
        }
