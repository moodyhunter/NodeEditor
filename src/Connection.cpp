...
Connection::Connection(PortType portType, Node &node, PortIndex portIndex) : _uid(QUuid::createUuid()), _outPortIndex(INVALID), _inPortIndex(INVALID), _connectionState()
{
...
Connection::Connection(Node &nodeIn, PortIndex portIndexIn, Node &nodeOut, PortIndex portIndexOut, TypeConverter typeConverter)
    : _uid(QUuid::createUuid()), _outNode(&nodeOut), _inNode(&nodeIn), _outPortIndex(portIndexOut), _inPortIndex(portIndexIn), _connectionState(),
...
NodeDataType Connection::dataType(PortType portType) const
{
    if (_inNode && _outNode)
    {
        auto model = (portType == PortType::In) ? _inNode->nodeDataModel() : _outNode->nodeDataModel();
