...
std::shared_ptr<Connection> FlowScene::restoreConnection(QJsonObject const &connectionJson)
{
    ...
    {
        connection = createConnection(*nodeIn, portIndexIn, *nodeOut, portIndexOut, getConverter());
    }
...
std::vector<Node *> FlowScene::allNodes() const
{
    ...
    std::transform(_nodes.begin(), _nodes.end(), std::back_inserter(nodes), [](std::pair<QUuid const, std::unique_ptr<Node>> const &p) { return p.second.get(); });
...
QByteArray FlowScene::saveToMemory(QJsonDocument::JsonFormat format) const
{
...
QJsonObject FlowScene::saveToObject() const
{
...
void FlowScene::loadFromObject(const QJsonObject &data)
{
...
QByteArray FlowScene::copyNodes(const std::vector<QtNodes::Node *> &nodes) const
{
    QJsonObject sceneJson;
    QSet<QUuid> nodeIds;
    QJsonArray nodesJsonArray;
