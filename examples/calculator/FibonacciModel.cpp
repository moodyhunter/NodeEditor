...
FibonacciModel::FibonacciModel() : NodeDataModel(), _indexCounter(1)
{
}

unsigned int FibonacciModel::nPorts(QtNodes::PortType portType) const
{
...
bool FibonacciModel::hasDynamicPorts(QtNodes::PortType portType) const
{
...
QtNodes::NodeDataType FibonacciModel::dataType(QtNodes::PortType, QtNodes::PortIndex) const
{
    return DecimalData().type();
}

std::shared_ptr<QtNodes::NodeData> FibonacciModel::outData(QtNodes::PortIndex port)
{
    return std::make_shared<DecimalData>(fibo(port));
}

void FibonacciModel::setInData(std::shared_ptr<QtNodes::NodeData>, QtNodes::PortIndex)
{
}

QString FibonacciModel::caption() const
{
    return QStringLiteral("Fibonacci");
}

bool FibonacciModel::portCaptionVisible(QtNodes::PortType, QtNodes::PortIndex) const
{
    return true;
}

QString FibonacciModel::portCaption(QtNodes::PortType, QtNodes::PortIndex portIndex) const
{
    return QString("Fibo %1").arg(portIndex);
}

QString FibonacciModel::name() const
{
    return QStringLiteral("Fibonacci");
}

QtNodes::NodeValidationState FibonacciModel::validationState() const
{
    return _modelValidationState;
}

QString FibonacciModel::validationMessage() const
{
